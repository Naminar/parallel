...
/* START*/
    ...
    /* Compute rows */
    for (int row_idx = 2; row_idx < ISIZE; ++row_idx) {
        ...
        for (int local_j = 0; local_j < local_j_max - 3; local_j++) { /* Compute independent cells */
            local_results[row_idx * work_size + local_j] = sin(5 * local_results[(row_idx - 2) * work_size + (local_j + 3)]);
        }
        ...
        if (world_rank != (world_size - 1)) { /* Compute dependent cell */
            ...
            local_results[row_idx * work_size + (local_j_max - 1)] = sin(5 * dependency);
...
/* Gather data */
    ...
    if (world_rank == 0) {
        ...
        FILE* ff = fopen("result_fly.txt","w");
